 ...
 */
public class BookedRentalFragment extends Fragment implements ItemClickCallback {
...
//    List<RentalResponse> bookedList=new ArrayList<>();
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if(loginResponse.getRoles().get(0).equals("ROLE_LESSOR")){
            System.out.println("im in fragmnt lessor booked");
            System.out.println(EStatus.BOOKED.toString());
            rentalService.getAllOrdersByUserIdAndStatus(Long.parseLong(loginResponse.getId()),EStatus.BOOKED.toString(),loginResponse.getToken(),lessorOrderResponseCallback());

        }else if(loginResponse.getRoles().get(0).equals("ROLE_LESSEE")){
            rentalService.getAllRentalsByUserIdAndStatus(Long.parseLong(loginResponse.getId()), EStatus.BOOKED.toString(), loginResponse.getToken(),lesseeOrderResponseCallback());
