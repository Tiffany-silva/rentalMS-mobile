 ...
 */
public class PickedRentalFragment extends Fragment implements ItemClickCallback {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if(loginResponse.getRoles().get(0).equals("ROLE_LESSOR")){
            ...
            rentalService.getAllOrdersByUserIdAndStatus(Long.parseLong(loginResponse.getId()),EStatus.PICKED.toString(),loginResponse.getToken(),lessorOrderResponseCallback());

        }else if(loginResponse.getRoles().get(0).equals("ROLE_LESSEE")){
            rentalService.getAllRentalsByUserIdAndStatus(Long.parseLong(loginResponse.getId()), EStatus.PICKED.toString(), loginResponse.getToken(),lesseeOrderResponseCallback());
